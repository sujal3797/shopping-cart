...
let calculation = () => {
    ...
    cartIcon.innerHTML = basket.map((x)=>x.item).reduce((acc, y)=> acc+y, 0)
}
...
let generateCardItems = () => {
    if(basket.length !== 0){
        ...
        const anchorTag = '<a href="index.html"><button class="HomeBtn">Back to Home</button></a>';
        label.innerHTML += anchorTag;
    }
};
